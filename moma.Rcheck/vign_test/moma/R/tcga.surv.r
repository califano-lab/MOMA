...
get.clin <- function(clin.file = NULL) {
    ...
    if (is.null(clin.file)) {
        ...
        if (length(clin.dir) == 0) {
            print("Error: can't find clinical data directory!")
            q()
        }
...
tibble.survfit_select <- function(clustering, clinical.tibble, progression.free.surv = FALSE) {
    ...
    list(survObj = clin.tibble$survObj, pval.best_v_worst = pval.best_worst, pval.overall = pval.overall, best.clus = best.surv.cluster, worst.clus = worst.surv.cluster)
}
