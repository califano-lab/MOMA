...
empirical.p <- function(gene.name, x) {
	...
	if (!(as.character(gene.name) %in% names(x))) { return (1.0) }
...
conditional.p <- function(gene.name, condition.on, x) {
	...
	# unranked genes in either distribution should get no score from this
	if (!(as.character(gene.name) %in% names(condition.on))) { return (1) }
	if (!(as.character(gene.name) %in% names(x))) { return (1) }
...
conditional.model <- function(viper.scores, diggit.scores, pathway.scores) {
	...
	integrated.pvals <- unlist(lapply(names(viper.scores), function(VIP) {
		...
		# for each type, produce p-values for DIGGIT as well as each dependent pathway or pathway 
		# based algorithm (including CINDy)
		all.pvals <- c(viper.p)
